...
def test_update_robot_turn_left():
    ...
    updated_robot = robots.update_robot(initial_robot, "turn-left", {"x": 10, "y": 10})
    assert updated_robot.bearing == "west"

def test_update_robot_turn_right():
    initial_robot = robots.Robot("north", 0, 0)
    updated_robot = robots.update_robot(initial_robot, "turn-right", {"x": 10, "y": 10})
    assert updated_robot.bearing == "east"

def test_update_robot_move_forward():
    initial_robot = robots.Robot("north", 0, 0)
    updated_robot = robots.update_robot(initial_robot, "move-forward", {"x": 10, "y": 10})
...
def test_update_robot_move_forward_bounds():
    initial_robot = robots.Robot("north", 9, 9)  # Near the edge of a 10x10 asteroid.
    updated_robot = robots.update_robot(initial_robot, "move-forward", {"x": 10, "y": 10})
    assert updated_robot.x == 9  # Should not exceed the asteroid's bounds.
    assert updated_robot.y == 10

def test_update_robot_missing_asteroid_data():
    initial_robot = robots.Robot("north", 0, 0)
    with pytest.raises(KeyError):
        robots.update_robot(initial_robot, "move-forward", {})
...
def test_load_commands_invalid_json():
    ...
    with pytest.raises(json.decoder.JSONDecodeError):
        list(robots.load_commands(f"{TEST_FILE_LOC}/test_instructions_invalid.txt"))
...
def test_load_commands_multiple_robots():
    ...
    with open("tests/test_instructions_multiple.txt", mode="w", encoding="utf-8") as test_file:
        test_file.writelines(commands)
    test_robots = list(robots.load_commands(f"{TEST_FILE_LOC}/test_instructions_multiple.txt"))
